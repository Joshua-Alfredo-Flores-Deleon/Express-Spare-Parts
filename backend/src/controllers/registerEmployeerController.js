...
registerEmployeerController.register = async (req, res) => {
  ...
  const { name, email, phone, hireDate, birthday, rol, user, password, status, isVerified} = req.body;

  try {
    //Validar que el correo no exista en la base de datos
    const existsEmployees = await employeesModel.findOne({ email });
    if (existsEmployees) {
      return res.status(400).json({ message: "Employees already exists" });
    }
...
registerEmployeerController.verifyCode = async (req, res) => {
  try {
    ...
    return res.status(200).json({message: "Employees registered"})
